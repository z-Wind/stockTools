...
def plot_pyramid(
    title: str, df_男: pd.DataFrame, df_女: pd.DataFrame, regions: list[str], y: list
) -> None:
    def visible(region):
        if "總計" in regions:
            if region == "總計":
...
def _build_category_buttons(kinds: list) -> list:
    """3.12：從 kinds 清單建立下拉按鈕設定（從 index_原始值_年增率_plot 拆出）。"""
    ...
    for i, kind in enumerate(kinds):
        ...
        arr = [i <= k < j for k in range(len(kinds))]
        buttons.append({"args": [{"visible": arr}], "label": kind, "method": "restyle"})

    return buttons


def _prepare_index_plots(
    df: pd.DataFrame,
    key: str,
    title_suffix: str,
    additional_layout: Optional[Dict],
) -> tuple[dict, dict]:
    """3.12：準備原始值/年增率/IRR 所需的 pivot DataFrame 與 layout。"""
    df = df.copy()
    ...
    buttons = _build_category_buttons(kinds)
    updatemenus = [
        {
            "x": 0.5,
            "y": 1.09,
    ...
    pivot_raw_dropna = pivot_raw.dropna(axis="index")
    實質購買力_df = pivot_raw_dropna.iloc[0] / pivot_raw_dropna * 100.0
    ...
    irr_df = pivot_datetime.apply(irr, axis=0).to_frame()
    irr_df.columns = ["IRR"]
    irr_df = irr_df.T

    pivot_yoy = df[df["TYPE"] == "年增率(%)"].pivot_table(
        index="TIME_PERIOD", columns="Item", values="Item_VALUE", sort=False
    )
    ...
    return {
        ...
        "irr_df": irr_df,
        "pivot_yoy": pivot_yoy,
        "pivot_raw_dropna": pivot_raw_dropna,
        "pivot_datetime": pivot_datetime,
...
def index_原始值_年增率_plot(
    ...
    """3.12：拆分為資料準備（_prepare_index_plots）與圖表建構兩個職責。"""
    key = sanitize_filename(key)
    df = df_get()

    d = _prepare_index_plots(df, key, title_suffix, additional_layout)
    base_layout = d["base_layout"]

    plots[f"{key}_原始值"] = plot_line(
        d["pivot_raw"],
        f"{key} 原始值 {title_suffix} {d['date_range']}",
        base_layout,
    )

    pivot_raw_dropna = d["pivot_raw_dropna"]
    plots[f"{key}_實質購買力"] = plot_line(
        d["實質購買力_df"],
        f"{key} 實質購買力=當月份的一百元等於 {pivot_raw_dropna.index[0]} 多少錢 "
        f"{pivot_raw_dropna.index[0]}~{pivot_raw_dropna.index[-1]}",
        base_layout,
    )

    pivot_datetime = d["pivot_datetime"]
    plots[f"{key}_IRR"] = plot_bar(
        d["irr_df"],
        f"{key} IRR(%) {title_suffix} {pivot_datetime.index[0][1]}~{pivot_datetime.index[-1][1]}",
        base_layout | {"yaxis": {"tickformat": ".2%"}},
    )

    additional_layout_年增率 = {"yaxis": {"tickformat": ".2%"}}
    if additional_layout:
        additional_layout_年增率 = merge_dict(
            additional_layout_年增率, copy.deepcopy(additional_layout)
        )
    plots[f"{key}_年增率"] = plot_line(
        d["pivot_yoy"] / 100,
        f"{key} 年增率(%) {title_suffix} {d['date_range']}",
...
def plot_消費者物價基本分類指數(plots) -> None:
    index_原始值_年增率_plot(
        plots,
        key="消費者物價基本分類指數",
...
def plot_生產者物價基本分類指數(plots) -> None:
    index_原始值_年增率_plot(
        plots,
        key="生產者物價基本分類指數",
...
def plot_躉售物價基本分類指數(plots) -> None:
    index_原始值_年增率_plot(
        plots,
        key="躉售物價基本分類指數",
...
def plot_人力資源調查縣市別失業率(plots) -> None:
    key = "人力資源調查縣市別失業率"
    key = sanitize_filename(key)
    df = df_人力資源調查縣市別失業率()
...
def plot_歷年人力資源調查重要指標(plots) -> None:
    key = "歷年人力資源調查重要指標"
    key = sanitize_filename(key)
    df = df_歷年人力資源調查重要指標()
...
def plot_人力資源調查重要指標(plots) -> None:
    key = "人力資源調查重要指標"
    key = sanitize_filename(key)
    df = df_人力資源調查重要指標()
    num_columns = [
...
def plot_歷年教育程度別失業率(plots) -> None:
    key = "歷年教育程度別失業率"
    key = sanitize_filename(key)
    df = df_歷年教育程度別失業率()
...
def plot_教育程度別失業率_按年齡分(plots) -> None:
    key = "教育程度別失業率—按年齡分"
    key = sanitize_filename(key)
    df = df_教育程度別失業率_按年齡分()
    num_columns = [
...
def plot_主要國家零歲平均餘命(plots) -> None:
    key = "主要國家零歲平均餘命"
    key = sanitize_filename(key)
    df = df_主要國家零歲平均餘命()
...
def plot_國民所得統計_國民所得_儲蓄與投資_季(plots) -> None:
...
def plot_國民所得統計_常用資料_季(plots) -> None:
    key = "國民所得統計-常用資料-季"
    key = sanitize_filename(key)
    df = df_國民所得統計_常用資料_季()
...
def plot_家庭收支調查_家庭收支重要指標(plots) -> None:
    key = "家庭收支調查-家庭收支重要指標"
    key = sanitize_filename(key)
...
def plot_家庭收支調查_所得總額按來源別分(plots) -> None:
...
def plot_家庭收支調查_所得總額與可支配所得(plots) -> None:
...
def plot_家庭收支調查_所得收入者人數與按年齡組別及性別之分配(plots) -> None:
...
def plot_家庭收支調查_戶內人數別平均每戶所得總額(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶經常性支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶儲蓄(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶內人數別平均每戶儲蓄率(plots) -> None:
...
def plot_家庭收支調查_戶內人數別平均每戶受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭戶數按戶內人口規模別之分配(plots) -> None:
...
def plot_家庭收支調查_農家平均每戶所得總額按來自農業與非農業分(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_農家與非農家平均每戶可支配所得_所得總額及儲蓄(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_農家與非農家平均每戶所得收入總計及受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶所得總額(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶經常性支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶儲蓄(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位之平均每戶儲蓄率(plots) -> None:
...
def plot_家庭收支調查_戶數五等分位之平均每戶受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位組之平均每戶人口數(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數五等分位組之平均每戶就業人數(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_戶數十等分位組分界點之可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者五等分位平均每人可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者十等分位組分界點之可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶所得總額(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶經常性支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶儲蓄(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_家庭組織型態別平均每戶儲蓄率(plots) -> None:
...
def plot_家庭收支調查_家庭組織型態別平均每戶受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_性別統計指標_家戶狀況按家庭組織型態分(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶所得總額(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶經常性支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶儲蓄(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_各縣市別平均每戶儲蓄率(plots) -> None:
...
def plot_家庭收支調查_各縣市別平均每戶受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者各縣市別平均每人所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者各縣市別平均每人非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者各縣市別平均每人可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者各縣市別平均每人受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_性別統計指標_戶數與結構按經濟戶長性別分(plots) -> None:
...
def plot_家庭收支調查_家庭戶數按所得總額組別及經濟戶長性別分(plots) -> None:
...
def plot_家庭收支調查_平均每戶可支配所得按經濟戶長性別分(plots) -> None:
    年_plot(
        plots,
        key="家庭收支調查-平均每戶可支配所得按經濟戶長性別分",
        df_get=df_家庭收支調查_平均每戶可支配所得按經濟戶長性別分,
        title_suffix="-元",
    )


def plot_家庭收支調查_性別統計指標_經濟戶長人數按性別_教育程度及年齡分(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_性別統計指標_單親戶長人數按性別_教育程度及年齡分(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶所得總額(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶經常性支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶儲蓄(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_經濟戶長職業別平均每戶儲蓄率(plots) -> None:
...
def plot_家庭收支調查_經濟戶長職業別平均每戶受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者職業別平均每人所得收入總計(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者職業別平均每人非消費支出(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者職業別平均每人可支配所得(plots) -> None:
    年_plot(
        plots,
...
def plot_家庭收支調查_所得收入者職業別平均每人受僱人員報酬(plots) -> None:
    年_plot(
        plots,
...
def plot_公開資訊觀測站_財務報告附註揭露之員工福利_薪資_資訊(plots) -> None:
...
def plot_公開資訊觀測站_非擔任主管職務之全時員工薪資資訊(plots) -> None:
...
def plot_企業ESG資訊揭露彙總資料_人力發展(plots) -> None:
    ...
    plots[f"{key}_非擔任主管職務之全時員工薪資_排序"] = plot_lines_bars(
        df_薪資[["平均數", "中位數"]].sort_values(["中位數", "平均數"], ascending=False).dropna(),
...
def plot_歷年受僱員工每人每月總薪資平均數(plots) -> None:
...
def plot_歷年受僱員工每人每月經常性薪資平均數(plots) -> None:
...
def plot_工業及服務業全體受僱員工全年總薪資統計表(plots) -> None:
...
def plot_工業及服務業受僱員工全年總薪資中位數及分布統計結果(plots) -> None:
...
def plot_工業及服務業每人每月工時_時_(plots) -> None:
    key = "工業及服務業每人每月工時(時)"
    key = sanitize_filename(key)
    df = df_受僱員工每人每月工時()
...
def plot_各業廠商調升經常性薪資參考各項因素之廠商比率_按行業分(plots) -> None:
...
def plot_各業廠商調升員工經常性薪資之廠商與員工人數比率_按行業分(plots) -> None:
...
def plot_公司合併報表董事酬金相關資訊(plots) -> None:
    key = "公司合併報表董事酬金相關資訊"
    key = sanitize_filename(key)
...
def plot_公司合併報表監察人酬金相關資訊(plots) -> None:
    key = "公司合併報表監察人酬金相關資訊"
    key = sanitize_filename(key)
...
def plot_綜稅總所得各縣市申報統計分析表(plots) -> None:
    key = "綜稅總所得各縣市申報統計分析表"
    key = sanitize_filename(key)
...
def plot_綜稅綜合所得總額全國各縣市鄉鎮村里統計分析表(plots) -> None:
...
def plot_財政統計年報_綜合所得稅結算申報_按淨所得級距別分(plots) -> None:
...
def plot_勞工退休金提繳統計年報_按地區_行業及規模別(plots) -> None:
...
def plot_勞工退休準備金專戶餘額統計(plots) -> None:
    key = "勞工退休準備金專戶餘額統計"
    key = sanitize_filename(key)
    df = df_勞工退休準備金專戶餘額統計()
...
def plot_勞工退休準備金提撥率統計(plots) -> None:
    key = "勞工退休準備金提撥率統計"
    key = sanitize_filename(key)
    df = df_勞工退休準備金提撥率統計()
...
def plot_勞工退休金提繳單位_提繳人數_提繳工資_提繳金額概況(plots) -> None:
...
def plot_勞工退休金平均提繳工資_按行業別(plots) -> None:
...
def plot_勞工退休金平均提繳工資_按年齡組別(plots) -> None:
...
def plot_新制勞工退休基金歷年最近月份收益率(plots) -> None:
...
def plot_全國賦稅收入實徵淨額日曆年別_按稅目別與地區別分(plots) -> None:
...
def plot_全國賦稅收入實徵淨額與預算數之比較(plots, items) -> None:
...
def plot_主計總處統計專區_家庭收支調查_統計表_常用資料(plots) -> None:
...
def plot_進口貿易值_按洲別___國別分_CY2001__(plots) -> None:
    年月混合_plot(
        plots,
...
def plot_出口貿易值_按洲別___國別分_CY2001__(plots) -> None:
    年月混合_plot(
        plots,
...
def plot_進出口貿易值_按國際商品統一分類制度_HS_及主要國別分(plots) -> None:
...
def plot_進口值_按主要貨品分(plots) -> None:
    年月混合_plot(
        plots,
        key="進口值_按主要貨品分",
...
def plot_出口值_按主要貨品分(plots) -> None:
    年月混合_plot(
        plots,
        key="出口值_按主要貨品分",
...
def plot_貿易指數_進口單位價值指數(plots) -> None:
    年月混合_plot(
        plots,
        key="貿易指數－進口單位價值指數",
...
def plot_貿易指數_出口單位價值指數(plots) -> None:
    年月混合_plot(
        plots,
        key="貿易指數－出口單位價值指數",
...
def plot_貿易指數_進口數量指數(plots) -> None:
    年月混合_plot(
        plots,
        key="貿易指數－進口數量指數",
...
def plot_貿易指數_出口數量指數(plots) -> None:
    年月混合_plot(
        plots,
        key="貿易指數－出口數量指數",
...
def plot_村里戶數_單一年齡人口_新增區域代碼_(plots) -> None:
...
def plot_現住人口性別_年齡_婚姻狀況_含同婚_(plots) -> None:
...
def plot_動態資料統計表_含同婚_(plots) -> None:
    key = "動態資料統計表（含同婚）"
    key = sanitize_filename(key)
    df = df_動態資料統計表()
...
def plot_結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記_(plots) -> None:
    ...
    # 【圖 2】：全國結婚率
    # ========================================================================
    df_現住人口_源頭 = df_現住人口性別_年齡_婚姻狀況()
    df_現住人口 = df_現住人口_源頭[df_現住人口_源頭["statistic_yyy"].isin(years)].copy()
    ...
    df_結婚率 = pd.DataFrame(index=df_結婚人數.index)
    df_結婚率["結婚率_不同性別"] = df_結婚人數.loc[:, ("不同性別",)].sum(
        axis=1
    ) / df_單身滿20人數.loc[:, (["未婚", "離婚_不同性別", "喪偶_不同性別"],)].sum(axis=1)
    df_結婚率["結婚率_相同性別"] = df_結婚人數.loc[:, ("相同性別",)].sum(
        axis=1
    ) / df_單身滿20人數.loc[:, (["未婚", "離婚_相同性別", "喪偶_相同性別"],)].sum(axis=1)
    df_結婚率["結婚率_相同性別_男"] = df_結婚人數.loc[:, ("相同性別", "男")] / df_單身滿20人數.loc[
        :, (["未婚", "離婚_相同性別", "喪偶_相同性別"], "男")
    ].sum(axis=1)
    df_結婚率["結婚率_相同性別_女"] = df_結婚人數.loc[:, ("相同性別", "女")] / df_單身滿20人數.loc[
    ...
    # ========================================================================
    # 【圖 4】：22 縣市分區結婚率（向量化高階重構，相容新版 Pandas 2.x）
    # ========================================================================
    ...
    del df_現住人口, df_現住人口_源頭  # 現住人口任務徹底結束，完全釋放
    gc.collect()
    ...
    df_區域別_單身滿20人數 = (
        df_區域別_單身人數.loc[:, cols_reg]
        .T.groupby(level=["縣市", "marital_status", "sex"])
        .sum()
        .T
    )
    del df_區域別_單身人數
    ...
    # 軸向矩陣高階運算（完美取代 22 次迴圈）
    # 不同性別
    mar_diff_v = (
        df_區域別_結婚人數.xs("不同性別", level="marriage_type", axis=1)
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    sng_diff_v = (
        df_區域別_單身滿20人數.loc[
            :, (slice(None), ["未婚", "離婚_不同性別", "喪偶_不同性別"], slice(None))
        ]
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_diff = mar_diff_v / sng_diff_v
    rate_diff.columns = [f"{col}_結婚率_不同性別" for col in rate_diff.columns]

    # 相同性別
    mar_same_v = (
        df_區域別_結婚人數.xs("相同性別", level="marriage_type", axis=1)
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    sng_same_v = (
        df_區域別_單身滿20人數.loc[
            :, (slice(None), ["未婚", "離婚_相同性別", "喪偶_相同性別"], slice(None))
        ]
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_same = mar_same_v / sng_same_v
    rate_same.columns = [f"{col}_結婚率_相同性別" for col in rate_same.columns]

    # 相同性別_男
    mar_same_m = (
        df_區域別_結婚人數.loc[:, (slice(None), "相同性別", "男")].T.groupby(level="縣市").sum().T
    )
    sng_same_m = (
        df_區域別_單身滿20人數.loc[
            :, (slice(None), ["未婚", "離婚_相同性別", "喪偶_相同性別"], "男")
        ]
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_same_m = mar_same_m / sng_same_m
    rate_same_m.columns = [f"{col}_結婚率_相同性別_男" for col in rate_same_m.columns]

    # 相同性別_女
    mar_same_f = (
        df_區域別_結婚人數.loc[:, (slice(None), "相同性別", "女")].T.groupby(level="縣市").sum().T
    )
    sng_same_f = (
        df_區域別_單身滿20人數.loc[
            :, (slice(None), ["未婚", "離婚_相同性別", "喪偶_相同性別"], "女")
        ]
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_same_f = mar_same_f / sng_same_f
    rate_same_f.columns = [f"{col}_結婚率_相同性別_女" for col in rate_same_f.columns]

    df_區域別_結婚率_最終 = pd.concat([rate_diff, rate_same, rate_same_m, rate_same_f], axis=1)

    plots[f"{key}_區域別_結婚率"] = plot_line(
        df_區域別_結婚率_最終.filter(like="結婚率", axis="columns"),
        f"{key}_區域別_結婚率 {df_區域別_結婚率_最終.index[0]}~{df_區域別_結婚率_最終.index[-1]}年",
        additional_layout={"yaxis": {"tickformat": ".2%"}},
    )
    ...
    # 最終全盤釋放
    del df_男女_年齡_婚姻類型, df
    gc.collect()


def plot_結婚對數按婚姻類型_性別及年齡分_按登記_(plots, items) -> None:
...
def plot_嬰兒出生數按性別_生母原屬國籍_地區__年齡及教育程度分_按登記_(plots) -> None:
...
def plot_嬰兒出生數按性別_生父原屬國籍_地區__年齡及教育程度分_按登記_(plots) -> None:
...
def plot_嬰兒出生數按嬰兒性別及生父母年齡分_按登記_(plots) -> None:
...
def plot_嬰兒出生數按生母年齡及出生身分分_按登記_(plots) -> None:
...
def plot_嬰兒出生數按性別_胎次及生母年齡分_按登記_(plots) -> None:
...
def plot_嬰兒胎次_vs_綜稅綜合所得總額(plots) -> None:
...
def plot_嬰兒出生數按嬰兒性別及出生胎別分_按登記_(plots) -> None:
...
def plot_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記_(plots) -> None:
    ...
    # 1. 載入並建立乾淨的基礎資料
    df_raw = df_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記()
    years = df_raw["statistic_yyy"].unique().tolist()

    # ========================================================================
    # 【補回圖 1】：總和趨勢圖
    # ========================================================================
    df_total = df_raw.pivot_table(
    ...
    # ========================================================================
    # 【補回圖 2】：全國不分區離婚率（優化版：獨立抽取，畫完立刻釋放）
    # ========================================================================
    df_離婚人數 = df_raw.pivot_table(
    ...
    df_現住_raw = df_現住人口性別_年齡_婚姻狀況()
    df_現住人口_全台 = df_現住_raw[df_現住_raw["statistic_yyy"].isin(years)][
        ["statistic_yyy", "population", "marital_status", "sex"]
    ].copy()

    df_已婚人數 = df_現住人口_全台.pivot_table(
        values="population",
        index="statistic_yyy",
    ...
    df_離婚率 = pd.DataFrame(index=df_離婚人數.index)
    df_離婚率["離婚率_不同性別"] = df_離婚人數.loc[:, ("不同性別",)].sum(axis=1) / df_已婚人數.loc[
        :, ("有偶_不同性別",)
    ].sum(axis=1)
    df_離婚率["離婚率_相同性別"] = df_離婚人數.loc[:, ("相同性別",)].sum(axis=1) / df_已婚人數.loc[
        :,
        (
            ("Ref_有偶_相同性別",)
            if ("有偶_相同性別",) not in df_已婚人數.columns
            else ("有偶_相同性別",)
        ),
    ].sum(
        axis=1
    ...
    df_離婚率["離婚率_相同性別_男"] = (
        df_離婚人數.loc[:, ("相同性別", "男")] / df_已婚人數.loc[:, ("有偶_相同性別", "男")]
    )
    df_離婚率["離婚率_相同性別_女"] = (
        df_離婚人數.loc[:, ("相同性別", "女")] / df_已婚人數.loc[:, ("有偶_相同性別", "女")]
    )
    ...
    # ========================================================================
    # 【原本圖 4】：22 縣市分區離婚率（向量化版：完美取代 22 次迴圈與 copy 記憶體地雷）
    # ========================================================================
    df_區域別_離婚人數 = df_raw.pivot_table(
        values="divorce_count",
        index="statistic_yyy",
        columns=["縣市", "marriage_type", "sex"],
        aggfunc="sum",
    ...
    df_現住人口_區域 = df_現住_raw[df_現住_raw["statistic_yyy"].isin(years)][
        ["statistic_yyy", "population", "marital_status", "sex", "縣市"]
    ].copy()
    del df_現住_raw  # 人口原始大表不再需要，釋放
    gc.collect()

    df_區域別_已婚人數 = df_現住人口_區域.pivot_table(
        values="population",
        index="statistic_yyy",
    ...
    # 1. 計算不同性別的總和
    div_diff = (
        df_區域別_離婚人數.xs("不同性別", level="marriage_type", axis=1)
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    mar_diff = (
        df_區域別_已婚人數.xs("有偶_不同性別", level="marital_status", axis=1)
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_diff = div_diff / mar_diff
    rate_diff.columns = [f"{col}_離婚率_不同性別" for col in rate_diff.columns]

    # 2. 計算相同性別的總和
    div_same = (
        df_區域別_離婚人數.xs("相同性別", level="marriage_type", axis=1)
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    mar_same = (
        df_區域別_已婚人數.xs("有偶_相同性別", level="marital_status", axis=1)
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_same = div_same / mar_same
    rate_same.columns = [f"{col}_離婚率_相同性別" for col in rate_same.columns]

    # 3. 計算相同性別_男
    div_same_m = (
        df_區域別_離婚人數.loc[:, (slice(None), "相同性別", "男")].T.groupby(level="縣市").sum().T
    )
    mar_same_m = (
        df_區域別_已婚人數.loc[:, (slice(None), "有偶_相同性別", "男")]
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_same_m = div_same_m / mar_same_m
    rate_same_m.columns = [f"{col}_離婚率_相同性別_男" for col in rate_same_m.columns]

    # 4. 計算相同性別_女
    div_same_f = (
        df_區域別_離婚人數.loc[:, (slice(None), "相同性別", "女")].T.groupby(level="縣市").sum().T
    )
    mar_same_f = (
        df_區域別_已婚人數.loc[:, (slice(None), "有偶_相同性別", "女")]
        .T.groupby(level="縣市")
        .sum()
        .T
    )
    rate_same_f = div_same_f / mar_same_f
    rate_same_f.columns = [f"{col}_離婚率_相同性別_女" for col in rate_same_f.columns]

    df_區域別_離婚率_最終 = pd.concat([rate_diff, rate_same, rate_same_m, rate_same_f], axis=1)

    plots[f"{key}_區域別_離婚率"] = plot_line(
        df_區域別_離婚率_最終.filter(like="離婚率", axis="columns"),
        f"{key}_區域別_離婚率 {df_區域別_離婚率_最終.index[0]}~{df_區域別_離婚率_最終.index[-1]}年",
        additional_layout={"yaxis": {"tickformat": ".2%"}},
    )
    ...
    # ========================================================================
    # 【其餘局部圖表】：隨畫隨刪管線（共 10 張圖，完整保留）
    # ========================================================================

    # 5. 婚姻類型
    df_婚姻類型 = df_raw.pivot_table(
        values="divorce_count",
        index="statistic_yyy",
        columns=["marriage_type", "sex"],
        aggfunc="sum",
    ...
    # 10. 女_年齡_縣市
    df_女_年齡_縣市 = df_raw[df_raw["sex"] == "女"].pivot_table(
    ...
    # 11. 男_年齡_縣市
    df_男_年齡_縣市 = df_raw[df_raw["sex"] == "男"].pivot_table(
        values="divorce_count", index="age", columns="縣市", aggfunc="sum", sort=False
    )
    plots[f"{key}_男_年齡_縣市"] = plot_line(
        df_男_年齡_縣市, f"{key}_男_年齡_縣市 {years[0]}~{years[-1]}年"
    ...
    # 12. 女_年齡_教育
    df_女_年齡_教育 = df_raw[df_raw["sex"] == "女"].pivot_table(
        values="divorce_count",
        index="age",
        columns=["edu", "marriage_type"],
        aggfunc="sum",
    ...
    # 13. 男_年齡_教育
    df_男_年齡_教育 = df_raw[df_raw["sex"] == "男"].pivot_table(
        values="divorce_count",
        index="age",
        columns=["edu", "marriage_type"],
        aggfunc="sum",
    ...
    # 14. 男女_年齡_婚姻類型
    df_男女_年齡_婚姻類型 = df_raw.pivot_table(
        values="divorce_count",
        index="age",
        columns=["sex", "marriage_type"],
        aggfunc="sum",
    ...
    # 最終深度釋放
    del df_男女_年齡_婚姻類型, df_raw
    gc.collect()


def plot_全國公立動物收容所收容處理情形統計表(plots, items) -> None:
...
def plot_定期定額交易戶數統計排行月報表(plots) -> None:
    key = "定期定額交易戶數統計排行月報表"
    key = sanitize_filename(key)
...
def plot_投信投顧公會基金費用比率(plots) -> None:
    key = "投信投顧公會基金費用比率"
    key = sanitize_filename(key)
    df = df_投信投顧公會基金費用比率()
...
def plot_基金績效評比(plots, items) -> None:
    key = "基金績效評比"
    key = sanitize_filename(key)
    df = df_基金績效評比()
...
# 建立一個特製的字典，專門用來攔截寫入操作
class StreamingPlotsDict(dict):

    def __init__(self, jsfolder: Path, plot_keys_list: List[str], lock: threading.Lock):
        super().__init__()
        self.jsfolder = jsfolder
        self.plot_keys_list = plot_keys_list
        self.lock = lock

    def __setitem__(self, key: str, item: str):
        """當任何 plot 函式執行 plots[key] = item 時，會觸發此方法。"""
        # 1. 立即渲染並壓縮（item 就是原本傳進來的 Plotly JSON 字串）
        graph = render_template("graph.js.j2", key=key, item=item)
        minified_graph = jsmin(graph)

        # 2. 立即寫入硬碟
        with open(self.jsfolder / f"{key}.js", "w", encoding="UTF-8") as f:
            f.write(minified_graph)

        # 3. 透過 Lock 確保多執行緒安全地記錄 key 名稱
        with self.lock:
            self.plot_keys_list.append(key)

        # 4. 關鍵：不要呼叫 super().__setitem__(key, item)！
        # 這樣巨大的 item (JSON) 就不會被留在記憶體中，函式結束後直接被垃圾回收（GC）

    def items(self):
        return [].items()  # 永遠回傳空，因為記憶體不留存資料

    def __len__(self):
        return len(self.plot_keys_list)
...
def main():
    ...
    # 準備用來收集所有成功生成的 key 的清單（取代原本的 plots 字典）
    plot_keys: List[str] = []
    plot_lock = threading.Lock()

    # 建立神奇的串流字典
    plots = StreamingPlotsDict(jsfolder, plot_keys, plot_lock)

    tasks = [
        # ========================================================================
        lambda p=plots: plot_消費者物價基本分類指數(p),
        lambda p=plots: plot_生產者物價基本分類指數(p),
        lambda p=plots: plot_躉售物價基本分類指數(p),
        lambda p=plots: plot_人力資源調查失業率(p),
        lambda p=plots: plot_人力資源調查縣市別失業率(p),
        lambda p=plots: plot_歷年人力資源調查重要指標(p),
        lambda p=plots: plot_人力資源調查重要指標(p),
        lambda p=plots: plot_歷年教育程度別失業率(p),
        lambda p=plots: plot_教育程度別失業率(p),
        lambda p=plots: plot_歷年年齡組別失業率(p),
        lambda p=plots: plot_年齡組別失業率(p),
        lambda p=plots: plot_教育程度別失業率_按年齡分(p),
        lambda p=plots: plot_就業率(p),
        lambda p=plots: plot_主要國家零歲平均餘命(p),
        lambda p=plots: plot_房價所得比(p),
        lambda p=plots: plot_貸款負擔率(p),
        # ========================================================================
        lambda p=plots: plot_國民所得統計_國民所得_儲蓄與投資_季(p),
        lambda p=plots: plot_國民所得統計_常用資料_季(p),
        lambda p=plots: plot_家庭收支調查_家庭收支重要指標(p),
        lambda p=plots: plot_家庭收支調查_所得總額按來源別分(p),
        lambda p=plots: plot_家庭收支調查_所得總額與可支配所得(p),
        lambda p=plots: plot_家庭收支調查_所得收入者人數與按年齡組別及性別之分配(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶所得總額(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶非消費支出(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶可支配所得(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶消費支出(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶經常性支出(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶儲蓄(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶儲蓄率(p),
        lambda p=plots: plot_家庭收支調查_戶內人數別平均每戶受僱人員報酬(p),
        lambda p=plots: plot_家庭收支調查_家庭戶數按戶內人口規模別之分配(p),
        lambda p=plots: plot_家庭收支調查_農家平均每戶所得總額按來自農業與非農業分(p),
        lambda p=plots: plot_家庭收支調查_農家與非農家平均每戶可支配所得_所得總額及儲蓄(p),
        lambda p=plots: plot_家庭收支調查_農家與非農家平均每戶消費支出_非消費支出及經常性支出(p),
        lambda p=plots: plot_家庭收支調查_農家與非農家平均每戶所得收入總計及受僱人員報酬(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶所得總額(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶非消費支出(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶可支配所得(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶消費支出(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶經常性支出(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶儲蓄(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶儲蓄率(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位之平均每戶受僱人員報酬(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位組之平均每戶人口數(p),
        lambda p=plots: plot_家庭收支調查_戶數五等分位組之平均每戶就業人數(p),
        lambda p=plots: plot_家庭收支調查_戶數十等分位組分界點之可支配所得(p),
        lambda p=plots: plot_家庭收支調查_所得收入者五等分位平均每人可支配所得(p),
        lambda p=plots: plot_家庭收支調查_所得收入者十等分位組分界點之可支配所得(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶所得總額(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶非消費支出(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶可支配所得(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶消費支出(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶經常性支出(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶儲蓄(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶儲蓄率(p),
        lambda p=plots: plot_家庭收支調查_家庭組織型態別平均每戶受僱人員報酬(p),
        lambda p=plots: plot_家庭收支調查_性別統計指標_家戶狀況按家庭組織型態分(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶所得總額(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶非消費支出(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶可支配所得(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶消費支出(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶經常性支出(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶儲蓄(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶儲蓄率(p),
        lambda p=plots: plot_家庭收支調查_各縣市別平均每戶受僱人員報酬(p),
        lambda p=plots: plot_家庭收支調查_所得收入者各縣市別平均每人所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_所得收入者各縣市別平均每人非消費支出(p),
        lambda p=plots: plot_家庭收支調查_所得收入者各縣市別平均每人可支配所得(p),
        lambda p=plots: plot_家庭收支調查_所得收入者各縣市別平均每人受僱人員報酬(p),
        lambda p=plots: plot_主計總處統計專區_家庭收支調查_統計表_常用資料(p),
        lambda p=plots: plot_家庭收支調查_性別統計指標_可支配所得按戶數五等分位經濟戶長性別比率(p),
        lambda p=plots: plot_家庭收支調查_性別統計指標_戶數與結構按經濟戶長性別分(p),
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得及消費支出依可支配所得按戶數五等分位分及經濟戶長年齡組別分(
            p
        ),
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得及消費支出依可支配所得按戶數五等分位分及經濟戶長教育程度別分(
            p
        ),
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得及消費支出依可支配所得按戶數五等分位分及經濟戶長性別分(
            p
        ),
        lambda p=plots: plot_家庭收支調查_家庭戶數按所得總額組別及經濟戶長性別分(p),
        lambda p=plots: plot_家庭收支調查_所得收入者人數按性別及可支配所得組別分(p),
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得按經濟戶長性別分(p),
        lambda p=plots: plot_家庭收支調查_性別統計指標_經濟戶長人數按性別_教育程度及年齡分(p),
        lambda p=plots: plot_家庭收支調查_性別統計指標_單親戶長人數按性別_教育程度及年齡分(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶所得總額(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶非消費支出(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶可支配所得(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶消費支出(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶經常性支出(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶儲蓄(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶儲蓄率(p),
        lambda p=plots: plot_家庭收支調查_經濟戶長職業別平均每戶受僱人員報酬(p),
        lambda p=plots: plot_家庭收支調查_所得收入者職業別平均每人所得收入總計(p),
        lambda p=plots: plot_家庭收支調查_所得收入者職業別平均每人非消費支出(p),
        lambda p=plots: plot_家庭收支調查_所得收入者職業別平均每人可支配所得(p),
        lambda p=plots: plot_家庭收支調查_所得收入者職業別平均每人受僱人員報酬(p),
        lambda p=plots: plot_家庭部門資產結構(p),
        lambda p=plots: plot_公開資訊觀測站_財務報告附註揭露之員工福利_薪資_資訊(p),
        lambda p=plots: plot_公開資訊觀測站_非擔任主管職務之全時員工薪資資訊(p),
        lambda p=plots: plot_企業ESG資訊揭露彙總資料_人力發展_vs_公司合併報表董事酬金相關資訊(p),
        lambda p=plots: plot_企業ESG資訊揭露彙總資料_人力發展(p),
        lambda p=plots: plot_歷年受僱員工每人每月總薪資平均數(p),
        lambda p=plots: plot_歷年受僱員工每人每月經常性薪資平均數(p),
        lambda p=plots: plot_工業及服務業全體受僱員工全年總薪資統計表(p),
        lambda p=plots: plot_工業及服務業受僱員工全年總薪資中位數及分布統計結果(p),
        lambda p=plots: plot_工業及服務業每人每月工時_時_(p),
        lambda p=plots: plot_各業廠商僱用職缺按月計薪者每人每月平均最低薪資_按職類及員工規模分(p),
        lambda p=plots: plot_各業廠商調升經常性薪資參考各項因素之廠商比率_按行業分(p),
        lambda p=plots: plot_各業廠商調升員工經常性薪資之廠商與員工人數比率_按行業分(p),
        lambda p=plots: plot_公司合併報表董事酬金相關資訊(p),
        lambda p=plots: plot_公司合併報表監察人酬金相關資訊(p),
        lambda p=plots: plot_綜稅總所得各縣市申報統計分析表(p),
        ...
        lambda p=plots: plot_勞工退休金平均提繳工資_按行業別(p),
        lambda p=plots: plot_勞工退休金平均提繳工資_按年齡組別(p),
        lambda p=plots: plot_新制勞工退休基金歷年最近月份收益率(p),
        # ========================================================================
        lambda p=plots: plot_全國賦稅收入實徵淨額日曆年別_按稅目別與地區別分(p),
        lambda p=plots, i=items: plot_全國賦稅收入實徵淨額與預算數之比較(p, i),
        # ========================================================================
        lambda p=plots: plot_進口貿易值_按洲別___國別分_CY2001__(p),
        lambda p=plots: plot_出口貿易值_按洲別___國別分_CY2001__(p),
        lambda p=plots: plot_進出口貿易值_按國際商品統一分類制度_HS_及主要國別分(p),
        lambda p=plots: plot_進口值_按主要貨品分(p),
        lambda p=plots: plot_出口值_按主要貨品分(p),
        lambda p=plots: plot_貿易指數_進口單位價值指數(p),
        lambda p=plots: plot_貿易指數_出口單位價值指數(p),
        lambda p=plots: plot_貿易指數_進口數量指數(p),
        lambda p=plots: plot_貿易指數_出口數量指數(p),
        # ========================================================================
        lambda p=plots: plot_村里戶數_單一年齡人口_新增區域代碼_(p),
        lambda p=plots: plot_現住人口性別_年齡_婚姻狀況_含同婚_(p),
        lambda p=plots, i=items: plot_結婚對數按婚姻類型_性別及年齡分_按登記_(p, i),
        lambda p=plots: plot_嬰兒出生數按性別_生母原屬國籍_地區__年齡及教育程度分_按登記_(p),
        lambda p=plots: plot_嬰兒出生數按性別_生父原屬國籍_地區__年齡及教育程度分_按登記_(p),
        lambda p=plots: plot_嬰兒出生數按嬰兒性別及生父母年齡分_按登記_(p),
        lambda p=plots: plot_嬰兒出生數按生母年齡及出生身分分_按登記_(p),
        lambda p=plots: plot_嬰兒出生數按性別_胎次及生母年齡分_按登記_(p),
        lambda p=plots: plot_嬰兒胎次_vs_綜稅綜合所得總額(p),
        lambda p=plots: plot_嬰兒出生數按嬰兒性別及出生胎別分_按登記_(p),
        # ========================================================================
        lambda p=plots, i=items: plot_全國公立動物收容所收容處理情形統計表(p, i),
        # ========================================================================
        lambda p=plots: plot_定期定額交易戶數統計排行月報表(p),
        lambda p=plots: plot_投信投顧公會基金費用比率(p),
        lambda p=plots: plot_集保戶股權分散表(p),
        # ========================================================================
    ]

    do_tasks(tasks, max_workers=4)

    del tasks
    gc.collect()

    print("\n" + "=" * 110)
    print("🚀 所有並行任務已結束。開始單獨執行高記憶體任務：")
    print("=" * 110)

    monster_definitions = [
        lambda p=plots, i=items: plot_基金績效評比(p, i),
        lambda p=plots: plot_動態資料統計表_含同婚_(p),
        lambda p=plots: plot_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記_(
            p
        ),
        lambda p=plots: plot_結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記_(p),
    ]

    # 逐一執行，確保前一個大魔王產生的所有暫存記憶體在下一個開始前「徹底死透」
    for monster_task_lambda in monster_definitions:
        # 用 max_workers=1 單一執行緒執行這單個任務
        do_tasks([monster_task_lambda], max_workers=1)

        # 【關鍵優化 2】這個大魔王跑完了，立刻手動引爆它的參照，並強制回收
        del monster_task_lambda
        for _ in range(2):
            gc.collect()

    del monster_definitions
    gc.collect()
...
def do_tasks(tasks, max_workers: int):
    ...
    _safe_print(f"開始執行所有繪圖任務（max_workers={max_workers}）...")

    def _get_func_name(task) -> str:
        # 兼容 lambda 與一般函式的名稱獲取
        try:
            return task.__code__.co_names[0]
    ...
    def _task_monitor_wrapper(actual_task, name: str):
        start_ram = _get_ram_usage()
        _safe_print(f"⏳ [開始執行] → {name} (當前系統總 RAM: {start_ram:.1f} MB)")

        try:
            # 1. 執行繪圖任務，其內部的大字串會觸發你的 StreamingPlotsDict.__setitem__
            result = actual_task()

            # 3. 任務剛結束的當下
            end_ram = _get_ram_usage()
            diff_ram = end_ram - start_ram
            _safe_print(
                f"🏁 [執行結束] ← {name} (結束時 RAM: {end_ram:.1f} MB | 本任務推高: {diff_ram:+.1f} MB)"
            )
            return result
        except Exception as e:
            raise e
        finally:
            # 2. 任務一結束，在執行緒內部立即做一次輕量垃圾回收
            # 這能確保該執行緒剛剛產生的區域變數（包含 Plotly 的中間字串）立即被銷毀
            gc.collect(1)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for task in tasks:
            func_name = _get_func_name(task)
            future = executor.submit(_task_monitor_wrapper, task, func_name)
            futures_map[future] = func_name

        # 邊完成邊釋放的核心邏輯
        for future in as_completed(list(futures_map.keys())):
            func_name = futures_map.pop(future, "未知任務")
            try:
                # 3. 取得結果後立刻丟棄，絕對不賦值給變數
                future.result()
                _safe_print(f"✅ 成功執行：{func_name}")
            except Exception as e:
                _safe_print(f"❌ 執行失敗：{func_name} ({type(e).__name__}): {e}")
                _safe_print(traceback.format_exc())
            finally:
                # 4. 極致的切斷參照
                # 很多時候記憶體不釋放是因為 future 物件內部保留了執行緒的 frame 資訊
                ...
                # 5. 每完成一個任務，就強制進行一次完整的垃圾回收
                gc.collect()
                _safe_print(f"當前系統 RAM: {_get_ram_usage():.1f} MB")
                _safe_print("-" * 30)

    futures_map.clear()
    _safe_print("所有任務執行完畢，進行最終深度記憶體回收...")

    # 6. 深度清理：連續回收確保循環參照被清空
    for _ in range(3):
        gc.collect()

    _safe_print(f"記憶體回收完成。最終系統 RAM: {_get_ram_usage():.1f} MB")
