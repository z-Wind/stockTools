...
def main():
    ...
    tasks = [
        ...
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得及消費支出依可支配所得按戶數五等分位分及經濟戶長年齡組別分(p),
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得及消費支出依可支配所得按戶數五等分位分及經濟戶長教育程度別分(p),
        lambda p=plots: plot_家庭收支調查_平均每戶可支配所得及消費支出依可支配所得按戶數五等分位分及經濟戶長性別分(p),
    ...
    monster_tasks = [
        ...
        lambda p=plots: plot_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記_(p),
        lambda p=plots: plot_工業及服務業每人每月工時_時_(p),
        lambda p=plots: plot_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記_(p),
    ]

    print("\n" + "=" * 110)
    print("🚀 所有並行任務已結束。開始「單線程、一隻一隻」執行高記憶體大魔王任務：")
    print("=" * 110)

    do_tasks(monster_tasks, max_workers=1)
...
def do_tasks(tasks, max_workers: int):
    ...
    Args:
        max_workers: 最大執行緒數，預設 8（I/O 密集型任務可略高於 CPU 核心數）。
                     若需回退為序列執行，設定為 1 即可。
    ...
    # 預先擷取函式名稱（在主執行緒，避免 co_names 在 lambda 中不準確）
    def _get_func_name(task) -> str:
        try:
            # lambda 的 co_names 包含所有被呼叫的名稱，取第一個
            names = task.__code__.co_names
            return names[0] if names else task.__qualname__
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures_map):
            func_name = futures_map[future]
            try:
                future.result()
                _safe_print(f"✅ 成功執行：{func_name}")
            except Exception as e:
                _safe_print(f"❌ 執行失敗：{func_name}")
                _safe_print(f"   錯誤類型：{type(e).__name__}")
                _safe_print(f"   錯誤訊息：{e}")
                _safe_print(traceback.format_exc())
            finally:
                _safe_print("-" * 30)

    _safe_print("所有任務執行完畢。")
