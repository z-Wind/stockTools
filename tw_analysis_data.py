...
def df_人力資源調查重要指標() -> pd.DataFrame:
    ...
    urls = {
        ...
        114: "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/236096/mp04037a114.xml",
    }
    xpath = "//人力資源調查重要指標"
...
def df_家庭收支調查_所得收入者五等分位平均每人可支配所得() -> pd.DataFrame:
    ...
    return _make_df_csv_simple(
        ...
        "年",
        r"可支配所得按所得收入者人數五等分位組之平均每人可支配所得-",
    )()


# https://data.gov.tw/dataset/11573 家庭收支調查-所得收入者十等分位組分界點之可支配所得
def df_家庭收支調查_所得收入者十等分位組分界點之可支配所得() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/024-%E6%89%80%E5%BE%97%E6%94%B6%E5%85%A5%E8%80%85%E5%8D%81%E7%AD%89%E5%88%86%E4%BD%8D%E7%B5%84%E5%88%86%E7%95%8C%E9%BB%9E%E4%B9%8B%E5%8F%AF%E6%94%AF%E9%85%8D%E6%89%80%E5%BE%97.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶所得總額() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/058-平均每戶所得總額按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶所得收入總計() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/059-平均每戶所得收入總計按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶非消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/062-平均每戶非消費支出按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶可支配所得() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/056-平均每戶可支配所得按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/061-平均每戶消費支出按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶經常性支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/063-平均每戶經常性支出按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶儲蓄() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/057-平均每戶儲蓄按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_家庭組織型態別平均每戶受僱人員報酬() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/060-平均每戶受僱人員報酬按家庭組織型態別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_性別統計指標_家戶狀況按家庭組織型態分() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
...
def df_家庭收支調查_各縣市別平均每戶所得總額() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/008-平均每戶所得總額按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_各縣市別平均每戶所得收入總計() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/009-平均每戶所得收入總計按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_各縣市別平均每戶非消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/021-平均每戶非消費支出按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_各縣市別平均每戶可支配所得() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
...
def df_家庭收支調查_各縣市別平均每戶消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
...
def df_家庭收支調查_各縣市別平均每戶經常性支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/022-平均每戶經常性支出按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_各縣市別平均每戶儲蓄() -> pd.DataFrame:
    url = (
...
def df_家庭收支調查_各縣市別平均每戶受僱人員報酬() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/010-平均每戶受僱人員報酬按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者各縣市別平均每人所得收入總計() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/081-所得收入者平均每人所得收入總計按區城別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者各縣市別平均每人非消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/083-所得收入者平均每人非消費支出按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者各縣市別平均每人可支配所得() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/080-所得收入者平均每人可支配所得按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者各縣市別平均每人受僱人員報酬() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/082-所得收入者平均每人受僱人員報酬按區域別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_性別統計指標_可支配所得按戶數五等分位經濟戶長性別比率() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/090-性別統計指標-可支配所得按戶數五等分位分之經濟戶長性別比率.csv",
        "年",
        r"經濟戶長比率",
    )()
...
def df_家庭收支調查_性別統計指標_戶數與結構按經濟戶長性別分() -> pd.DataFrame:
...
def df_家庭收支調查_家庭戶數按所得總額組別及經濟戶長性別分() -> pd.DataFrame:
...
def df_家庭收支調查_平均每戶可支配所得按經濟戶長性別分() -> pd.DataFrame:
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/018-%E5%B9%B3%E5%9D%87%E6%AF%8F%E6%88%B6%E5%8F%AF%E6%94%AF%E9%85%8D%E6%89%80%E5%BE%97%E6%8C%89%E7%B6%93%E6%BF%9F%E6%88%B6%E9%95%B7%E6%80%A7%E5%88%A5%E5%88%86.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_性別統計指標_經濟戶長人數按性別_教育程度及年齡分() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/055-%E6%80%A7%E5%88%A5%E7%B5%B1%E8%A8%88%E6%8C%87%E6%A8%99-%E7%B6%93%E6%BF%9F%E6%88%B6%E9%95%B7%E4%BA%BA%E6%95%B8%E6%8C%89%E6%80%A7%E5%88%A5%E3%80%81%E6%95%99%E8%82%B2%E7%A8%8B%E5%BA%A6%E5%8F%8A%E5%B9%B4%E9%BD%A1%E5%88%86.csv",
        "年",
        r"-人",
    )()
...
def df_家庭收支調查_性別統計指標_單親戶長人數按性別_教育程度及年齡分() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/084-%E6%80%A7%E5%88%A5%E7%B5%B1%E8%A8%88%E6%8C%87%E6%A8%99-%E5%96%AE%E8%A6%AA%E6%88%B6%E9%95%B7%E4%BA%BA%E6%95%B8%E6%8C%89%E6%80%A7%E5%88%A5%E3%80%81%E6%95%99%E8%82%B2%E7%A8%8B%E5%BA%A6%E5%8F%8A%E5%B9%B4%E9%BD%A1%E5%88%86.csv",
        "年",
        r"-人",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶所得總額() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/049-平均每戶所得總額按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶所得收入總計() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/050-平均每戶所得收入總計按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶非消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/053-平均每戶非消費支出按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶可支配所得() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/047-平均每戶可支配所得按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/052-平均每戶消費支出按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()


def df_家庭收支調查_經濟戶長職業別平均每戶經常性支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/054-平均每戶經常性支出按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶儲蓄() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/048-平均每戶儲蓄按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_經濟戶長職業別平均每戶受僱人員報酬() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/051-平均每戶受僱人員報酬按經濟戶長職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者職業別平均每人所得收入總計() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/086-所得收入者平均每人所得收入總計按職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者職業別平均每人非消費支出() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/088-所得收入者平均每人非消費支出按職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者職業別平均每人可支配所得() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/085-所得收入者平均每人可支配所得按職業別分.csv",
        "年",
        r"-元",
    )()
...
def df_家庭收支調查_所得收入者職業別平均每人受僱人員報酬() -> pd.DataFrame:
    """3.4：委派至 _make_df_csv_simple 工廠函式。"""
    return _make_df_csv_simple(
        "https://ws.dgbas.gov.tw/001/Upload/461/relfile/11525/232214/087-所得收入者平均每人受僱人員報酬按職業別分.csv",
        "年",
        r"-元",
    )()


# https://www.stat.gov.tw/cp.aspx?n=2773 家庭部門資產結構
def df_家庭部門資產結構() -> pd.DataFrame:
    key = "家庭部門資產結構"
    key = sanitize_filename(key)
    url = "https://ws.dgbas.gov.tw/001/Upload/463/relfile/10315/2653/113%E8%A1%A86.xlsx"

    def df_家庭部門資產結構_中間項目(excel_bytes) -> pd.DataFrame:
        """遍歷所有工作表，精準抽取「土地按市價重評價」的中間項目並合併。"""

        # 1. 定義你要讀取的目標工作表名稱清單
        sheets = ["109", "110", "111", "112", "113"]
        all_series = []  # 用來存放各年份的 Series

        for sheet in sheets:
            try:
                # 💡 精準打擊：
                # usecols="A,B,E" -> A=年(預防用), B=項目別, E=中間項目的「金額」
                ...
                df_sheet = df_sheet.dropna(subset=["年"])
                df_sheet["項目別"] = df_sheet["項目別"].astype(str).str.strip()
                df_sheet["項目別"] = df_sheet["項目別"].str.replace(r"[\n \r]", "", regex=True)

                # 將「項目別」設為 Index，這樣合併時 Pandas 就會自動對齊相同項目
                df_sheet = df_sheet.set_index("項目別")

                # 只保留我們需要的該年度金額這一欄 (Series)
                all_series.append(df_sheet[sheet])

            except Exception as e:
                print(f"⚠️ 讀取工作表 [{sheet}] 時發生錯誤: {e}")
                continue

        if not all_series:
            return pd.DataFrame()

        # 💡 核心拼裝：將 109-113 的 Series 橫向（axis=1）拼接成一個 DataFrame
        # Pandas 會自動以「項目別」當作 Index 主鍵完美對齊！
        df_final = pd.concat(all_series, axis="columns")

        # 確保 Index 的名字叫做 "項目別"
        df_final.index.name = "項目別"

        # 負債轉為負值
        df_final.iloc[13:16] = -df_final.iloc[13:16]

        df_final.columns = [int(col) + 1911 for col in df_final.columns]
...
def df_公開資訊觀測站_財務報告附註揭露之員工福利薪資資訊() -> pd.DataFrame:
...
def df_公開資訊觀測站_非擔任主管職務之全時員工薪資資訊() -> pd.DataFrame:
...
def df_企業ESG資訊揭露彙總資料_人力發展() -> pd.DataFrame:
...
def df_歷年受僱員工每人每月總薪資平均數() -> pd.DataFrame:
...
def df_歷年受僱員工每人每月經常性薪資平均數() -> pd.DataFrame:
...
def df_工業及服務業全體受僱員工全年總薪資統計表() -> pd.DataFrame:
    ...
    r = _get_session().get(url_按性別及教育程度分, verify=False)
    ...
    r = _get_session().get(url_按年齡別分, verify=False)
    ...
    r = _get_session().get(url_按員工特性, verify=False)
    ...
    r = _get_session().get(url_按年齡及教育程度分, verify=False)
    ...
    r = _get_session().get(url_按工作場所所在縣市別及年齡別分, verify=False)
...
def df_工業及服務業受僱員工全年總薪資中位數及分布統計結果() -> pd.DataFrame:
    key = "工業及服務業受僱員工全年總薪資中位數及分布統計結果"
    key = sanitize_filename(key)

    # 無法與 111 年前合併來看，因為統計方法有變
    # 需由新往舊遞減，以符合最新的項目順序
    urls = {
        113: [
        ...
        112: [
            "https://ws.dgbas.gov.tw/001/Upload/463/relfile/10980/234206/表3　工業及服務業全年總薪資中位數－全體受僱員工.xlsx",
            "https://ws.dgbas.gov.tw/001/Upload/463/relfile/10980/234206/表4　工業及服務業全年總薪資中位數－本國籍全時受僱員工.xlsx",
            "https://ws.dgbas.gov.tw/001/Upload/463/relfile/10980/234206/表5　工業及服務業四等分位組分界點之全年總薪資－全體受僱員工按特性別分.xlsx",
            "https://ws.dgbas.gov.tw/001/Upload/463/relfile/10980/234206/表6　工業及服務業四等分位組分界點之全年總薪資－本國籍全時受僱員工按特性別分.xlsx",
            "https://ws.dgbas.gov.tw/001/Upload/463/relfile/10980/234206/表2　工業及服務業十等分位組分界點之全年總薪資－全體受僱員工.xlsx",
        ],
    }
    ...
    df_四等分位組分界點之全年總薪資_全體受僱員工按特性別分_集合 = {}
    df_四等分位組分界點之全年總薪資_本國籍全時受僱員工按特性別分_集合 = {}
    df_十等分位組分界點之全年總薪資_全體受僱員工_集合 = []
    for year, url_list in urls.items():
        df_總薪資中位數_全體受僱員工按特性別分 = []
        if url_list[0]:
            for i in range(0, 2):
                data = read_excel_with_cache(
                    EXTRA_DATA_DIR / key / f"總薪資中位數_全體受僱員工按特性別分_{year}.xlsx.gz",
                    url_list[0],
                    lambda excel_bytes: pd.read_excel(
                        excel_bytes,
                        engine="calamine",
                ...
                data = data.filter(regex=r"^\d+年$", axis=1)
                data.columns = [int(col.split("年")[0]) + 1911 for col in data.columns]  # 轉西元
                data = data.replace("- |-", np.nan, regex=True)

                df_總薪資中位數_全體受僱員工按特性別分.append(data)

            df_總薪資中位數_全體受僱員工按特性別分 = pd.concat(
                df_總薪資中位數_全體受僱員工按特性別分
            )
            df_總薪資中位數_全體受僱員工按特性別分 *= 10000
            df_總薪資中位數_全體受僱員工按特性別分 = df_總薪資中位數_全體受僱員工按特性別分.astype(
                float
            )
            df_總薪資中位數_全體受僱員工按特性別分_集合.append(
                df_總薪資中位數_全體受僱員工按特性別分
            )
        ...
        if url_list[1]:
            ...
            for i in range(0, 2):
                data = read_excel_with_cache(
                    EXTRA_DATA_DIR
                    / key
                    / f"總薪資中位數_本國籍全時受僱員工按特性別分_{year}.xlsx.gz",
                    url_list[1],
                    lambda excel_bytes: pd.read_excel(
                        excel_bytes,
                        engine="calamine",
            ...
            df_總薪資中位數_本國籍全時受僱員工按特性別分 = (
                df_總薪資中位數_本國籍全時受僱員工按特性別分.astype(float)
            )
            df_總薪資中位數_本國籍全時受僱員工按特性別分_集合.append(
                df_總薪資中位數_本國籍全時受僱員工按特性別分
            )
        ...
        if url_list[2]:
            ...
            for i in range(0, 2):
                data = read_excel_with_cache(
                    EXTRA_DATA_DIR
                    / key
                    / f"四等分位組分界點之全年總薪資_全體受僱員工按特性別分_{year}.xlsx.gz",
                    url_list[2],
                    lambda excel_bytes: pd.read_excel(
                        excel_bytes,
                        engine="calamine",
                ...
                data = (
                    data.set_index("特性")
                    .T.drop_duplicates()
                    .T.dropna(axis="index", how="all")[["Q1", "中位數", "Q3", "平均"]]
                )
            ...
            df_四等分位組分界點之全年總薪資_全體受僱員工按特性別分_集合[year + 1911] = (
                df_四等分位組分界點之全年總薪資_全體受僱員工按特性別分
            )
        ...
        if url_list[3]:
            ...
            for i in range(0, 2):
                data = read_excel_with_cache(
                    EXTRA_DATA_DIR
                    / key
                    / f"四等分位組分界點之全年總薪資_本國籍全時受僱員工按特性別分_{year}.xlsx.gz",
                    url_list[3],
                    lambda excel_bytes: pd.read_excel(
                        excel_bytes,
                        engine="calamine",
            ...
            df_四等分位組分界點之全年總薪資_本國籍全時受僱員工按特性別分_集合[year + 1911] = (
                df_四等分位組分界點之全年總薪資_本國籍全時受僱員工按特性別分
            )
        ...
        if url_list[4]:
            ...
            data = read_excel_with_cache(
                EXTRA_DATA_DIR / key / f"十等分位組分界點之全年總薪資_全體受僱員工_{year}.xlsx.gz",
                url_list[4],
                lambda excel_bytes: pd.read_excel(
                    excel_bytes,
                    engine="calamine",
            ...
            data = (
                data.set_index("年")
                .T.drop_duplicates()
                .T.dropna(axis="index", how="all")[["D" + str(i) for i in range(1, 10)]]
            )
            ...
            df_十等分位組分界點之全年總薪資_全體受僱員工_集合.append(
                df_十等分位組分界點之全年總薪資_全體受僱員工
            )

    df_總薪資中位數_全體受僱員工按特性別分_集合 = pd.concat(
        df_總薪資中位數_全體受僱員工按特性別分_集合, axis="columns", join="inner"
    )
    df_總薪資中位數_全體受僱員工按特性別分_集合 = df_總薪資中位數_全體受僱員工按特性別分_集合.loc[
        :, ~df_總薪資中位數_全體受僱員工按特性別分_集合.columns.duplicated()
    ].sort_index(axis="columns")
    df_總薪資中位數_本國籍全時受僱員工按特性別分_集合 = pd.concat(
        df_總薪資中位數_本國籍全時受僱員工按特性別分_集合, axis=1, join="inner"
    )
    df_總薪資中位數_本國籍全時受僱員工按特性別分_集合 = (
        df_總薪資中位數_本國籍全時受僱員工按特性別分_集合.loc[
            :, ~df_總薪資中位數_本國籍全時受僱員工按特性別分_集合.columns.duplicated()
        ].sort_index(axis="columns")
    )
    df_四等分位組分界點之全年總薪資_全體受僱員工按特性別分_集合 = pd.concat(
        df_四等分位組分界點之全年總薪資_全體受僱員工按特性別分_集合
    )
    df_四等分位組分界點之全年總薪資_本國籍全時受僱員工按特性別分_集合 = pd.concat(
        df_四等分位組分界點之全年總薪資_本國籍全時受僱員工按特性別分_集合
    )
    df_十等分位組分界點之全年總薪資_全體受僱員工_集合 = pd.concat(
        df_十等分位組分界點之全年總薪資_全體受僱員工_集合, axis="index"
    )
    df_十等分位組分界點之全年總薪資_全體受僱員工_集合 = (
        df_十等分位組分界點之全年總薪資_全體受僱員工_集合.loc[
            ~df_十等分位組分界點之全年總薪資_全體受僱員工_集合.index.duplicated(), :
...
def df_受僱員工每人每月工時() -> pd.DataFrame:
    ...
    r = _get_session().get(url.format(first_year=first_year, year=year, month=month), verify=False)
    json_data = json.loads(r.content)
...
def df_各業廠商僱用職缺按月計薪者每人每月平均最低薪資_按職類及員工規模分() -> pd.DataFrame:
...
def df_各業廠商調升經常性薪資參考各項因素之廠商比率_按行業分() -> pd.DataFrame:
...
def df_各業廠商調升員工經常性薪資之廠商與員工人數比率_按行業分() -> pd.DataFrame:
...
def df_公司合併報表董事酬金相關資訊() -> pd.DataFrame:
...
def df_公司合併報表監察人酬金相關資訊() -> pd.DataFrame:
...
def df_綜稅總所得各縣市申報統計分析表() -> pd.DataFrame:
    key = "綜稅總所得各縣市申報統計分析表"
    key = sanitize_filename(key)
...
def df_綜稅綜合所得總額全國各縣市鄉鎮村里統計分析表() -> pd.DataFrame:
...
def df_財政統計年報_綜合所得稅結算申報_按淨所得級距別分() -> pd.DataFrame:
...
def df_勞工退休金提繳統計年報_按地區_行業及規模別() -> pd.DataFrame:
...
# F00 類別 33379
def df_勞工退休準備金專戶餘額統計() -> pd.DataFrame:
...
# https://apiservice.mol.gov.tw/OdService/openapi/OAS.html
# F00 類別 33379
def df_勞工退休準備金提撥率統計() -> pd.DataFrame:
...
def df_勞工退休金提繳單位_提繳人數_提繳工資_提繳金額概況() -> pd.DataFrame:
...
def df_勞工退休金平均提繳工資_按行業別() -> pd.DataFrame:
...
def df_勞工退休金平均提繳工資_按年齡組別() -> pd.DataFrame:
...
def df_新制勞工退休基金歷年最近月份收益率() -> pd.DataFrame:
...
def df_全國賦稅收入實徵淨額日曆年別_按稅目別與地區別分() -> pd.DataFrame:
...
def df_全國賦稅收入實徵淨額與預算數之比較() -> pd.DataFrame:
...
def df_主計總處統計專區_家庭收支調查_統計表_常用資料() -> pd.DataFrame:
    ...
    r = _get_session().get(url_平均每人月消費, verify=False)
    df_平均每人月消費 = get_data(r.content, [0, 1], 1)
    df_平均每人月消費["年別"] += 1911  # 轉西元

    r = _get_session().get(url_可支配所得平均數, verify=False)
    ...
    r = _get_session().get(url_可支配所得中位數, verify=False)
...
def df_進口貿易值_按洲別_國別分() -> pd.DataFrame:
...
# https://data.gov.tw/dataset/15392 進出口貿易值_按洲別 ∕ 國別分(CY2001~)
def df_出口貿易值_按洲別_國別分() -> pd.DataFrame:
...
def df_進出口貿易值_按國際商品統一分類制度_HS_及主要國別分() -> pd.DataFrame:
...
def df_進口值_按主要貨品分() -> pd.DataFrame:
...
def df_出口值_按主要貨品分() -> pd.DataFrame:
...
def df_貿易指數_進口單位價值指數() -> pd.DataFrame:
...
def df_貿易指數_出口單位價值指數() -> pd.DataFrame:
...
def df_貿易指數_進口數量指數() -> pd.DataFrame:
...
def df_貿易指數_出口數量指數() -> pd.DataFrame:
...
def df_村里戶數_單一年齡人口() -> pd.DataFrame:
...
def df_現住人口性別_年齡_婚姻狀況() -> pd.DataFrame:
    ...
    # 封裝一個管線函數，讓每一頁讀進來時立刻進行「記憶體極小化」處理
    def process_page_data(json_data) -> pd.DataFrame:
        ...
        # 4. 高效字串切片：取代原本爆記憶體的正則替換分割
        # 台灣行政區前三個字必定是縣市（如：臺北市、新北市、彰化縣）
        site_str = data["site_id"].str.strip()
        data["縣市"] = site_str.str[:3]
        data["鄉鎮"] = site_str.str[3:]

        # 5. 年份轉換（直接在這裡做，轉成 category 或 int 都可以，這裡維持轉西元字串）
        data["statistic_yyy"] = (data["statistic_yyy"].astype(int) + 1911).astype(str)

        # 6. 移除 site_id 欄位，任務已完成
    ...
    for year in range(108, datetime.today().year - 1911):
        ...
        # 處理第 1 頁
        processed_data = process_page_data(json_data)
        ...
        for page in range(2, pages + 1):
            ...
            # 串流處理每一頁，處理完立即覆蓋 json_data 變數釋放原始 JSON 記憶體
            processed_data = process_page_data(json_data)
            if processed_data is not None:
                df_list.append(processed_data)

            del json_data, processed_data

    print(f"正在合併 1300 萬行的人口壓縮矩陣... (當前清單碎片數: {len(df_list)})")

    # 執行最終合併（此時裡面的 DataFrame 體積都只有原本的 15% 左右，絕對不會爆）
    ...
    print(f"完成合併 1300 萬行的人口壓縮矩陣，並釋放記憶體")
...
def df_結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記() -> pd.DataFrame:
...
def df_結婚對數按婚姻類型_性別及年齡分_按登記() -> pd.DataFrame:
    key = "結婚對數按婚姻類型、性別及年齡分(按登記)"
    key = sanitize_filename(key)
    urls = {
        108: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/169B54F1-8C46-48FB-8E28-164755BE51F8/download",
        109: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/8BB88A9D-4F47-4798-9557-682D338923B9/download",
        110: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/4540FE37-A4FC-4832-9028-F7CD3734B160/download",
        111: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/70900CD1-5314-433D-BDE3-1DF5C971ECD1/download",
        112: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/7DECF6DE-7AD6-4EC8-85A9-D2B8B703A014/download",
        113: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/169B54F1-8C46-48FB-8E28-164755BE51F8/download",
        114: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/F094822C-634B-493A-A2BF-D095E520DD19/resource/090C78EC-B344-4825-9B9F-AF2B3D0194C6/download",
    }
...
def df_嬰兒出生數按性別_生母原屬國籍_地區_年齡及教育程度分_按登記() -> pd.DataFrame:
...
def df_嬰兒出生數按性別_生父原屬國籍_地區_年齡及教育程度分_按登記() -> pd.DataFrame:
    key = "嬰兒出生數按性別、生父原屬國籍（地區）、年齡及教育程度分(按登記)"
    key = sanitize_filename(key)
    urls = {
        107: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/5BAD1943-66B8-4641-93C2-E782756EBDA1/download",
        108: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/4188B9D4-195C-423B-A936-0A61D9AE5F01/download",
        109: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/58ED2363-BC18-4667-8D00-26C2AF9CB35C/download",
        110: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/43DBD613-B4E8-4EBB-BD35-5B79E58166F9/download",
        111: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/1C52FAFC-FEFC-4B10-8E51-81CD91CCB649/download",
        112: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/F1806618-D721-426B-8A51-95C96AE728CF/download",
        114: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/B07AA35A-A44F-4DEE-A4D0-30EF41F4511F/resource/404FBFBE-9A03-47C2-873C-BC61FB39DA2C/download",
    }
...
def df_嬰兒出生數按嬰兒性別及生父母年齡分_按登記() -> pd.DataFrame:
...
def df_嬰兒出生數按生母年齡及出生身分分_按登記() -> pd.DataFrame:
    key = "嬰兒出生數按生母年齡及出生身分分(按登記)"
    key = sanitize_filename(key)
    urls = {
        107: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/94BFACA3-69FD-4A0A-BEB9-44D0EC534415/download",
        108: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/CCB9654B-B722-44A3-8A95-84D56B048F10/download",
        109: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/0C7B49C4-2B84-434E-86D5-E933F9199B90/download",
        110: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/401AD23D-9B8C-4180-B559-55D28B7F1FEA/download",
        111: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/FC12D25D-AB81-45D8-8815-0E60181EFE45/download",
        112: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/068CCDD8-DFEE-464A-A55B-90DB70388ECC/download",
        113: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/6AF685B1-DA07-4E8C-AFE0-C59A3FB06021/download",
        114: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4AB18950-C58F-4DF0-8CA8-1019567CD91A/resource/45EE7D09-CE18-4380-A42C-5072ED55BED8/download",
    }
...
def df_嬰兒出生數按性別_胎次及生母年齡分_按登記() -> pd.DataFrame:
    key = "嬰兒出生數按性別、胎次及生母年齡分(按登記)"
    key = sanitize_filename(key)
    urls = {
        106: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/E2E702A0-EA3B-4689-8CA9-DD9083E15534/download",
        107: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/5F11E0E0-8F23-4F95-B187-E15F6EF8AAE6/download",
        108: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/87673B9A-31D7-4900-9D7D-62C715EB54AC/download",
        109: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/8B7C2D82-9F36-452C-86F5-2CD370C0CCA4/download",
        110: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/205DD511-67C3-464C-8B47-A273B1258F61/download",
        111: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/D7B783A1-90C2-4C8B-B143-20DBC826F4C2/download",
        112: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/A5E5E762-D38E-463F-9262-0CFFCE2E5AE7/download",
        113: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/1858BBBA-82EC-4E00-8C77-0E206E4CCED2/download",
        114: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/4B775EF8-DEC8-47B7-9ABC-A9B39E427F31/resource/76F3EC04-A928-45A0-8893-30F4761E098A/download",
    }
...
def df_嬰兒出生數按嬰兒性別及出生胎別分_按登記() -> pd.DataFrame:
    key = "嬰兒出生數按嬰兒性別及出生胎別分(按登記)"
    key = sanitize_filename(key)
    urls = {
        110: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/9E4690DB-249B-4378-A0F6-DE3CA502FC6C/resource/C89BA952-9140-47C5-805A-34F3A0773978/download",
        111: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/9E4690DB-249B-4378-A0F6-DE3CA502FC6C/resource/CBD76D1D-2609-4553-9B8E-06BA912249B8/download",
        112: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/9E4690DB-249B-4378-A0F6-DE3CA502FC6C/resource/C7432B38-55F0-4DF1-8C91-33770B7A823C/download",
        113: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/9E4690DB-249B-4378-A0F6-DE3CA502FC6C/resource/A6B28B8E-F601-452F-925A-75A6320A633F/download",
        114: "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/9E4690DB-249B-4378-A0F6-DE3CA502FC6C/resource/7326A7B5-D7CD-4060-81F5-97ED4D9115EE/download",
    }
...
def df_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記() -> pd.DataFrame:
...
def df_全國公立動物收容所收容處理情形統計表() -> pd.DataFrame:
...
def df_定期定額交易戶數統計排行月報表() -> pd.DataFrame:
    key = "定期定額交易戶數統計排行月報表"
    key = sanitize_filename(key)
...
def df_銀行間市場新臺幣對美元收盤匯率() -> pd.DataFrame:
    key = "銀行間市場新臺幣對美元收盤匯率"
    key = sanitize_filename(key)
...
def df_投信投顧公會基金費用比率() -> pd.DataFrame:
    key = "投信投顧公會基金費用比率"
    key = sanitize_filename(key)
    ...
    with requests.Session() as s:
        s.headers = _get_session().headers
...
def df_基金績效評比() -> pd.DataFrame:
    ...
    def get_data(year, month) -> pd.DataFrame:
        ...
        if needs_download:
            ...
            time.sleep(2)
            try:
                r = _get_session().get(url, headers=headers, timeout=15)
...
def update():
    tasks = [
        ...
        lambda: df_動態資料統計表(),
        lambda: df_結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記(),
        lambda: df_離婚_終止結婚人數按婚姻類型_性別_年齡_原屬國籍_地區_及教育程度分_按登記(),
        lambda: df_嬰兒出生數按性別_生母原屬國籍_地區_年齡及教育程度分_按登記(),
        lambda: df_嬰兒出生數按嬰兒性別及生父母年齡分_按登記(),
        lambda: df_定期定額交易戶數統計排行月報表(),
        lambda: df_綜稅總所得各縣市申報統計分析表(),
        ...
        lambda: df_教育程度別失業率(),
        lambda: df_年齡組別失業率(),
        lambda: df_教育程度別失業率_按年齡分(),
        lambda: df_家庭部門資產結構(),
        lambda: df_各業廠商僱用職缺按月計薪者每人每月平均最低薪資_按職類及員工規模分(),
        lambda: df_各業廠商調升經常性薪資參考各項因素之廠商比率_按行業分(),
        lambda: df_各業廠商調升員工經常性薪資之廠商與員工人數比率_按行業分(),
        lambda: df_結婚對數按婚姻類型_性別及年齡分_按登記(),
        lambda: df_嬰兒出生數按性別_生父原屬國籍_地區_年齡及教育程度分_按登記(),
        lambda: df_嬰兒出生數按生母年齡及出生身分分_按登記(),
        lambda: df_嬰兒出生數按性別_胎次及生母年齡分_按登記(),
        lambda: df_嬰兒出生數按嬰兒性別及出生胎別分_按登記(),
        lambda: df_歷史_勞工退休金提繳統計年報_按地區_行業及規模別(),
        lambda: df_勞工退休金提繳單位_提繳人數_提繳工資_提繳金額概況(),
